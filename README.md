...
- **智能搜索**：输入本子 ID 即可获取详细信息（名称、页数、章节、标签）与封面缩略图，
  并附可点击的网页链接
- **探索**：按关键词搜索本子，可切换按作品 / 作者 / 标签 / 角色搜索，支持 `+包含` `-排除` 语法；
  结果每页 20 个、按窗口宽度自动换行，点击封面可放大，点击名称进入本子详情
- **本子详情**：展示大封面与标题 / ID / 作者 / 标签 / 页数 / 章节数，
...
主界面顶栏的「探索」按钮可打开探索页，用关键词搜索本子：

1. 在顶部搜索框左侧选择搜索方式：作品 / 作者 / 标签 / 角色
...
5. 点击名称进入「本子详情」页：左上方是大封面，右上方是标题 / 本子 ID / 作者 / 标签 / 页数 / 章节数，
   下方可以「跳转官网」「直接下载」或「加入下载列表」；返回探索页后搜索结果仍然保留
...
│   ├── explore_page.py         # 探索页（关键词搜索 + 结果网格 + 封面放大）
