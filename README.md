# Jm2PDF v2.3.1 - 禁漫本子探索、在线浏览与下载工具 📚
...
- **本子详情**：展示大封面与标题 / ID / 作者 / 标签 / 点赞数 / 观看数 / 页数 / 章节数，
  并提供「跳转官网」「直接下载」「加入下载列表」「浏览」四个操作；
  进入页面即自动取前 5 页预览（点击可放大，放大后可用两侧按钮逐张翻阅）
- **在线浏览**：详情页点「浏览」即可直接在本子里翻页阅读，不必先下载 ——
  支持横式（单页居中、左右翻页）与竖式（整本从上到下无缝滚动）、显示当前页 / 总页数、
  右上角关闭；按需取图并预取后续页面，页数多的本子按段加载，不会一次性拉满
- **批量下载**：支持逗号/空格分隔的多个 ID，一次下载多个本子
- **合并 PDF**：每个章节可合并为 PDF 文件，通过「生成 PDF」开关自由关闭；
...
- **本地浏览**：选中已下载的 PDF 或图片文件夹后点「浏览」，即可在软件内直接阅读 ——
  与在线浏览同一套界面（横竖式切换、翻页、页码 / 文件名、右上角关闭）
- **邮件推送**：可选将生成的 PDF 通过邮件发送（支持 QQ/163 等大多数邮箱）
